...
import { CANVAS_W, CANVAS_H, ROOMS } from "./constants.js";
import { ROLE_LABEL } from "../auth/roles.js";
import { logout as logoutRequest, fetchUsers } from "../auth/api.js";
...
export default function Office({ me, onLogout }) {
  const displayName = me.email.split("@")[0];
  const { canvasRef, room, messages, videos, status, myId, tasks, send, sendChat, setTyping, switchRoom } =
    useRoom({ room: "lobby", displayName });
  const [boardOpen, setBoardOpen] = useState(false);
  const [users, setUsers] = useState([]); // user ที่สมัครทั้งหมด → selector มอบหมาย task
  ...
  return (
    <div className="app">
      <div className="topbar">
        ...
        <select className="room-select" value={room} onChange={(e) => switchRoom(e.target.value)}>
          {ROOMS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}
        </select>
        <span className="spacer" />
        <button className="ghost" onClick={() => setBoardOpen(true)}>📋 Board</button>
        <span className="user-chip"><span className="dot" />{displayName} · {ROLE_LABEL[me.role] || me.role}</span>
      ...
      <button className="help-fab" onClick={() => alert(HELP_TEXT)} title="วิธีเล่น">?</button>
