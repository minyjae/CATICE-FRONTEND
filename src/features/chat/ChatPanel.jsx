...
const TABS = [["private", "ส่วนตัว"], ["room", "ห้องนี้"], ["all", "ทั้งหมด"]];
...
export default function ChatPanel({ messages, myId, onSend, onTypingChange }) {
  ...
  return (
    <div className="panel chat">
      <div className="chat-tabs">
        {TABS.map(([v, l]) => (
          <button key={v} className={"chat-tab" + (tab === v ? " active" : "")} onClick={() => setTab(v)}>{l}</button>
        ))}
      </div>
          ...
          return (
            <div className="msg-item" key={i}>
              <div className="msg-head">
                ...
                <span className="msg-time">{new Date(m.t).toLocaleTimeString("th-TH", { hour: "2-digit", minute: "2-digit" })}</span>
              </div>
